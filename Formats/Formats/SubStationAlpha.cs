...
namespace AHD.SM.Formats
{
    public class SubStationAlpha : SubtitlesFormat
    {
        ...
        public override void Save(string filePath, Encoding encoding)
        {
            ...
            {
                string current = "Dialogue: Marked=0," + TimeFormatConvertor.To_TimeSpan_Milli(this.SubtitleTrack.Subtitles[i].StartTime, ".", MillisecondLength.N2).Substring(1, 10) + ","
                     + TimeFormatConvertor.To_TimeSpan_Milli(this.SubtitleTrack.Subtitles[i].EndTime, ".", MillisecondLength.N2).Substring(1, 10) + ",Default,NTP,0000,0000,0000,!Effect,"
